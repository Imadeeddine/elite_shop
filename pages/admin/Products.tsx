...
const AdminProducts: React.FC = () => {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      // Check file size (limit 2MB for base64 storage)
      if (file.size > 2 * 1024 * 1024) {
        return alert("حجم الصورة كبير جداً، يرجى اختيار صورة أقل من 2 ميغابايت");
      }
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="flex gap-4 pt-6">
                <button
                  type="submit"
                  disabled={isSaving}
                  className={`flex-1 ${isSaving ? 'bg-slate-400' : 'bg-indigo-600'} text-white py-5 rounded-3xl font-black text-lg shadow-xl shadow-indigo-200 transition-all active:scale-95`}
                >
                  {isSaving ? 'جاري الحفظ...' : 'حفظ فوراً'}
                </button>
